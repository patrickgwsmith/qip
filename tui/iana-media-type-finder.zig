//! Search IANA media types and their registration references.
...
fn matches(media_type: data.MediaType) bool {
...
fn matchCount() usize {
    var count: usize = 0;
    for (data.media_types) |media_type| {
        if (matches(media_type)) count += 1;
    }
    return count;
}

fn selectedMediaType() ?data.MediaType {
    var index: usize = 0;
    for (data.media_types) |media_type| {
        if (!matches(media_type)) continue;
        if (index == selected) return media_type;
        index += 1;
    }
    return null;
}
...
fn listRow(writer: *Writer, media_type: data.MediaType, active: bool) void {
    writer.text(if (active) "> " else "  ");
    writer.text(media_type.name);
    if (media_type.qualifier.len > 0) {
        writer.text("  ");
        writer.text(media_type.qualifier);
    }
    writer.newline();
}
...
fn renderList(writer: *Writer) void {
    ...
    writer.text("MEDIA TYPE FINDER  IANA ");
    writer.text(data.updated);
    writer.text("  ");
    writer.number(count);
    writer.text(" / ");
    writer.number(data.media_types.len);
    writer.newline();
    ...
    writer.text("  Registered media type");
    writer.newline();
    ...
    if (count == 0) {
        ...
        for (data.media_types) |media_type| {
            ...
            if (match_index >= first and match_index < first + visible) {
                listRow(writer, media_type, match_index == selected);
            }
            match_index += 1;
        }
    }
...
fn renderDetails(writer: *Writer) void {
    const media_type = selectedMediaType() orelse return renderList(writer);
    writer.text("MEDIA TYPE DETAILS  Enter or Esc: back");
    writer.newline();
    writer.text(media_type.name);
    writer.newline();
    detailLine(writer, "IANA label", if (media_type.qualifier.len == 0) "No qualifier" else media_type.qualifier);
    detailLine(writer, "References", media_type.references);
    detailLine(writer, "Record date", media_type.date);
    detailLine(writer, "IANA updated", data.updated);
    writer.text("This registry list does not map file extensions.");
}
