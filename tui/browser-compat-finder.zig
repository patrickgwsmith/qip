//! Search MDN Web API and CSS property browser compatibility data.
...
fn listRow(writer: *Writer, feature: data.Feature, active: bool) void {
    ...
    writer.text(feature.path);
    writer.newline();
}

fn renderList(writer: *Writer) void {
    const count = matchCount();
    writer.text("BROWSER COMPAT  BCD ");
    ...
    writer.text("  Web API or CSS property feature");
    writer.newline();
...
fn renderDetails(writer: *Writer) void {
    ...
    writer.newline();
    _ = wrapText(writer, feature.path, 0, writer.width);
    writer.text("  Browser     Support  (* conditions; select for details)");
    writer.newline();
    ...
    writer.text(" statements:");
    writer.newline();
    _ = wrapText(writer, feature.details[browser], note_scroll, writer.width);
}
    ...
    if (details) {
        ...
        switch (x11_key) {
            ...
            XK_PAGE_UP, XK_PAGE_DOWN => {
                ...
                const path_rows = wrapText(null, feature.path, 0, columns);
                const visible = @max(1, lines -| (8 + path_rows));
                const note_rows = wrapText(null, feature.details[browser], 0, columns);
