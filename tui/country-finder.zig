//! Search country names, ISO codes, currencies, and calling prefixes.
...
fn matches(country: data.Country) bool {
...
fn matchCount() usize {
    var count: usize = 0;
    for (data.countries) |country| {
        if (matches(country)) count += 1;
    }
    return count;
}

fn selectedCountry() ?data.Country {
    var index: usize = 0;
    for (data.countries) |country| {
        if (!matches(country)) continue;
        if (index == selected) return country;
        index += 1;
    }
    return null;
}
...
fn listRow(writer: *Writer, country: data.Country, active: bool) void {
    writer.text(if (active) "> " else "  ");
    writer.text(country.code);
    writer.padTo(6);
    if (writer.width >= 70) {
        writer.text(country.name);
        writer.padTo(37);
        writer.text(if (country.currencies.len == 0) "-" else country.currencies);
        writer.padTo(51);
        writer.text(if (country.dial.len == 0) "-" else country.dial);
    } else {
        writer.text(country.name);
        writer.text("  ");
        writer.text(if (country.currencies.len == 0) "-" else country.currencies);
...
fn renderList(writer: *Writer) void {
    ...
    writer.text("COUNTRY FINDER  ");
    writer.number(count);
    writer.text(" / ");
    writer.number(data.countries.len);
    writer.newline();
    ...
    if (writer.width >= 70) {
        writer.text("  ISO Country");
        writer.padTo(37);
        writer.text("Currency");
        writer.padTo(51);
        writer.text("Calling prefix");
    } else {
        writer.text("  ISO Country  Currency  Calling prefix");
    }
    writer.newline();
    ...
    if (count == 0) {
        ...
        for (data.countries) |country| {
            ...
            if (match_index >= first and match_index < first + visible) {
                listRow(writer, country, match_index == selected);
            }
            match_index += 1;
        }
    }
...
fn renderDetails(writer: *Writer) void {
    const country = selectedCountry() orelse return renderList(writer);
    writer.text("COUNTRY DETAILS  Enter or Esc: back");
    writer.newline();
    writer.text(country.name);
    writer.text("  (");
    writer.text(country.code);
    writer.text(" / ");
    writer.text(country.alpha3);
    writer.text(")");
    writer.newline();
    detailLine(writer, "Currency", country.currencies);
    detailLine(writer, "Calling prefix", country.dial);
    detailLine(writer, "Capital", country.capital);
    detailLine(writer, "Region", country.region);
    detailLine(writer, "Domain", country.tld);
}
