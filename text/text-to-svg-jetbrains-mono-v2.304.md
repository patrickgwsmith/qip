# Text To SVG With JetBrains Mono 2.304

`text-to-svg-jetbrains-mono-v2.304.wasm` renders plain UTF-8 text as wrapped
JetBrains Mono paths on a transparent SVG canvas. `measure` sets the maximum
line width and the SVG width. The component derives the SVG height from the
font metrics, line count, and line height. It does not add padding, shrink
text, truncate text, or vertically center the result. The paths inherit
`currentColor`.

This version uses the official JetBrains Mono NL Regular and Bold faces. They
do not include code ligatures. The output contains paths rather than `<text>`,
so it has no external font dependency. Every supported glyph has the same
advance.

```bash
printf '%s' 'A monospaced text layer without ligatures.' |
  qip run text/text-to-svg-jetbrains-mono-v2.304.wasm \
...
- `line_height_em`: baseline advance as a multiple of `font_size`. The default
  is the font's authored 1.32em line height. Zero places every line on the same
...
```bash
printf '%s' 'A colored monospaced text layer.' |
  qip run \
    text/text-to-svg-jetbrains-mono-v2.304.wasm \
...
16 lines with ASCII, Latin-1 except soft hyphen, and selected punctuation. It
rejects invalid UTF-8, unsupported codepoints, excess lines, and glyphs that
cannot fit within the measure. Use another renderer for bidirectional text or
scripts that require contextual shaping.
